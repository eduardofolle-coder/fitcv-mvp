...
import { buildHardData, hardDataForPrompt, type HardData } from '../services/cvComposer.js';
import { explainJudgment, factsCorpus, judgeStatement } from '../services/narrativeVerifier.js';
import {
  classifyField,
  ...
  type Classification,
  type Resolution,
} from '../services/fieldClassifier.js';
...
async function loadHardData(userId: string): Promise<HardData> {
  ...
  if (profile.experience === null || profile.experience === undefined) {
    throw new AppError(409, 'Your CV was analyzed before FITCV stored your work history in structured form. Please upload it again.');
  }
...
async function draftAnswers(
  pending: Pending[],
  hard: HardData,
  job: Record<string, string> | undefined,
  userId: string
...
router.post(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    const job = rawJob && typeof rawJob === 'object'
      ? {
          title: String(rawJob.title ?? '').slice(0, 200),
          company: String(rawJob.company ?? '').slice(0, 200),
          description: String(rawJob.description ?? '').slice(0, 8000),
        }
      : undefined;

    const hard = await loadHardData(req.user.id);
    ...
    if (pending.length > 0) {
      const drafted = await draftAnswers(pending, hard, job, req.user.id);
    ...
    res.json({ success: true, data: { resolutions, summary } });
  })
);
